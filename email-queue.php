...
if ( ! function_exists( 'mlq_admin_default_setup' ) ) {
	function mlq_admin_default_setup() {
		...
		$settings_hook = add_submenu_page( 'bws_panel', 'Email Queue', 'Email Queue', 'manage_options', 'mlq_settings', 'mlq_admin_settings_content' );
 ...
 * @return void
 */
if ( ! function_exists( 'mlq_plugins_loaded' ) ) {
	function mlq_plugins_loaded() {
...
if ( ! function_exists ( 'mlq_init' ) ) {
	function mlq_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $mlq_plugin_info, '3.9' );
	}
}
		...
		if ( ! isset( $mlq_options['plugin_option_version'] ) || $mlq_options['plugin_option_version'] != $mlq_plugin_info["Version"] ) {
			...
			foreach ( $default_plugins as $def_plugin_key => $def_plugin_value ) {
				...
				if ( ! array_key_exists( $def_plugin_key, $plugins_in_db ) ) {
					...
					$wpdb->insert( $wpdb->base_prefix . 'mlq_mail_plugins',
						array(
			...
			/* deleting smtp settings option as deprecated sinse 1.0.9 */
			if ( 'smtp' == $mlq_options['mail_method'] ) {
				$mlq_options['mail_method'] = 'wp_mail';
			}
			if ( isset( $mlq_options['smtp_settings'] ) ) {
				unset( $mlq_options['smtp_settings'] );
			}
			$mlq_options_default['display_settings_notice'] = 0;

			/* updating install link if db version less than 0.3 */
			if ( ! isset( $mlq_options['plugin_db_version'] ) || $mlq_options['plugin_db_version'] != $mlq_db_version ) {
				if ( 1 == version_compare( '0.3', $mlq_options['plugin_db_version'] ) ) {
					foreach ( $default_plugins as $def_plugin_key => $def_plugin_value ) {
						/* updating install link for free plugins only */
						if ( '0' == $def_plugin_value['pro_status'] || '1' == $def_plugin_value['pro_status'] ) {
							$wpdb->update(
								$wpdb->base_prefix . 'mlq_mail_plugins',
								array(
									'install_link'	=> $def_plugin_value['install_link']
								),
								array(
				...
				$mlq_options['plugin_db_version'] = $mlq_db_version;
			}
...
if ( ! function_exists ( 'mlq_register_plugin_links' ) ) {
	function mlq_register_plugin_links( $links, $file ) {
		...
		if ( $file == $base ) {
			...
			$links[] = '<a href="https://support.bestwebsoft.com" target="_blank">' . __( 'FAQ', 'email-queue' ) . '</a>';
			$links[] = '<a href="https://support.bestwebsoft.com" target="_blank">' . __( 'Support', 'email-queue' ) . '</a>';
		}
		return $links;
	}
}
...
if ( ! function_exists ( 'mlq_get_default_plugins' ) ) {
	function mlq_get_default_plugins() {
		$install_link_prefix = network_admin_url( 'plugin-install.php?tab=search' );
		...
		$default_plugins = array(
			'contact-form-plugin/contact_form.php' => array(
				...
				'install_link'			=> $install_link_prefix . '&type=term&s=Contact+Form+bestwebsoft&plugin-search-input=Search+Plugins',
				'pro_status'			=> 1,
			...
			'sender/sender.php' => array(
				...
				'install_link'			=> $install_link_prefix . '&s=Sender+Bestwebsoft&plugin-search-input=Search+Plugins',
				'pro_status'			=> 1,
			...
			'subscriber/subscriber.php' => array(
				...
				'install_link'			=> $install_link_prefix . '&s=Subscriber+Bestwebsoft&plugin-search-input=Search+Plugins',
				'pro_status'			=> 1,
			...
			'updater/updater.php' => array(
				...
				'install_link'			=> $install_link_prefix . '&type=term&s=updater+bestwebsoft&plugin-search-input=Search+Plugins',
				'pro_status'			=> 1,
			...
			'contact-form-pro/contact_form_pro.php' => array(
				...
				'install_link'			=> 'https://bestwebsoft.com/products/wordpress/plugins/contact-form/?k=b35edff0787994c4f1ca3684d9e2190d',
				'pro_status'			=> 2,
			...
			'sender-pro/sender-pro.php' => array(
				...
				'install_link'			=> 'https://bestwebsoft.com/products/wordpress/plugins/sender/?k=acd2f126ee86524e5f28f1f4abd4fbae',
				'pro_status'			=> 2,
			...
			'subscriber-pro/subscriber-pro.php' => array(
				...
				'install_link'			=> 'https://bestwebsoft.com/products/wordpress/plugins/subscriber/?k=fed5c082f5d3d1e5c9a60337d78bc339',
				'pro_status'			=> 2,
			...
			'updater-pro/updater_pro.php' => array(
				...
				'install_link'			=> 'https://bestwebsoft.com/products/wordpress/plugins/updater/?k=707f68de65a07bdd80407a5c1dff0108',
				'pro_status'			=> 2,
...
if ( ! function_exists( 'mlq_get_mail_data_from_contact_form' ) ) {
	function mlq_get_mail_data_from_contact_form( $plugin_link, $sendto, $message_subject, $message_text, $attachments, $mlq_user_email = false, $headers = '' ) {
		...
		if ( 1 == $plugin_info['in_queue_status'] ) {
			...
			*/
			if ( empty( $headers ) && ! empty( $mlq_user_email ) ) {
				/* free version has not $mlq_user_email parametr */
				$headers = $mlq_user_email;
				$mlq_user_email = false;
			}
			if ( empty( $headers ) ) {
				...
				$headers = ( isset( $cntctfrm_options['html_email'] ) && 1 == $cntctfrm_options['html_email'] ) ?
				...
				/* Add reply-to */
				if ( isset( $cntctfrm_options['header_reply_to'] ) && 1 == $cntctfrm_options['header_reply_to'] )
					$headers .= 'Reply-To: ' . $mlq_user_email . "\n";
				/* Additional headers */
				$headers .= ( isset( $cntctfrm_options['from_email'] ) && 'custom' == $cntctfrm_options['from_email'] ) ? 'From: ' . stripslashes( $cntctfrm_options['custom_from_email'] ) . '' : 'From: ' . $mlq_user_email . '';
			}
			/* end @todo */
...
if ( ! function_exists( 'mlq_get_mail_data_from_sender' ) ) {
	function mlq_get_mail_data_from_sender( $plugin_link, $users_to, $message_subject, $message_text, $mail_id_in_sender ) {
		...
		/* get plugin id, in_queue_status and priority from DB based on plugin link*/
		$plugin_info = $wpdb->get_row( $wpdb->prepare( "SELECT `mail_plugin_id`, `in_queue_status`, `priority_general` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `plugin_link`='%s';", $plugin_link ), ARRAY_A );
		if ( empty( $sndr_options ) ) {
			$sndr_options = ( is_multisite() ) ? get_site_option( 'sndr_options' ) : get_option( 'sndr_options' );
		}
		/* save mail and put it in queue if plugin has in_queue_status 'ON' */
		if ( $plugin_info && ! empty( $sndr_options ) && 1 == $plugin_info['in_queue_status'] ) {
			$plugin_id = $plugin_info['mail_plugin_id'];
			$mail_priority = $plugin_info['priority_general'];
			/* get sender options for headers */
			$from_name  = $sndr_options['sndr_from_custom_name'];
			...
			foreach ( $users_to as $user_to ) {
				$user_email = $wpdb->get_var( $wpdb->prepare( "SELECT `user_email` FROM `" . $wpdb->base_prefix . "sndr_mail_users_info` WHERE `id_user`=%s;", $user_to ) );
				$wpdb->insert(
					$wpdb->base_prefix . 'mlq_mail_users',
					array(
...
if ( ! function_exists( 'mlq_start_mailout_from_sender_pro' ) ) {
	function mlq_start_mailout_from_sender_pro( $plugin_link, $mailout_id ) {
		...
		if ( 1 == $plugin_info['in_queue_status'] ) {
			...
			$mailout_data_sender_pro = $wpdb->get_row( $wpdb->prepare(
				"SELECT `mail_id`, `distridution_lists_ids`, `use_plugin_settings`, `from_name`, `from_email`, `mailout_priority_number`, `mailout_create`, `remote_delivery` FROM `" . $wpdb->base_prefix . "sndr_mailout` WHERE `mailout_id`='%d';", $mailout_id ), ARRAY_A );

			if ( '1' == $mailout_data_sender_pro['remote_delivery'] ) {
				/* get headers for message */
				if ( '1' != $mailout_data_sender_pro['use_plugin_settings'] ) {
					if ( empty( $sndr_options ) )
						$sndr_options = ( is_multisite() ) ? get_site_option( 'sndr_options' ) : get_option( 'sndr_options' );

					$from_name  = $sndr_options['from_custom_name'];
					$from_email = empty( $sndr_options['from_email'] ) ? get_option( 'admin_email' ) : $sndr_options['from_email'];
				} else {
					$from_name  = $mailout_data_sender_pro['from_name'];
					$from_email = $mailout_data_sender_pro['from_email'];
				}
				$headers = 'From: ' . $from_name . ' <' . $from_email . '>' . "\r\n";

				/* get message subject and text from Sender Pro DB table*/
				$message_data_in_sender_pro = $wpdb->get_row( $wpdb->prepare( "SELECT `subject`, `body` FROM `" . $wpdb->base_prefix . "sndr_mail_send` WHERE `mail_send_id`='%d';", $mailout_data_sender_pro['mail_id'] ), ARRAY_A );

				/* Save mail message into database */
				$wpdb->insert(
					$wpdb->base_prefix . 'mlq_mail_send',
					array(
						'subject'			=> $message_data_in_sender_pro['subject'],
						'body'				=> $message_data_in_sender_pro['body'],
						'headers'			=> $headers,
						'date_create'		=> strtotime( $mailout_data_sender_pro['mailout_create'] ),
						'plugin_id'			=> $plugin_id,
						'priority'			=> $mail_priority,
						'priority_sndrpr'	=> $mailout_data_sender_pro['mailout_priority_number'],
					)
				);
				$last_id_inserted = $wpdb->insert_id;

				/* Save email addresses into database */
				$users_to = mlq_get_user_ids_from_sender_pro( $mailout_data_sender_pro['distridution_lists_ids'] );
				foreach ( $users_to as $user_to ) {
					$user_email = $wpdb->get_var( $wpdb->prepare( "SELECT `user_email` FROM `" . $wpdb->base_prefix . "sndr_mail_users_info` WHERE `id_user`=%s;", $user_to) );
					$wpdb->insert(
						$wpdb->base_prefix . 'mlq_mail_users',
 ...
 */
if ( ! function_exists( 'mlq_get_user_ids_from_sender_pro' ) ) {
	function mlq_get_user_ids_from_sender_pro( $mailing_ids ) {
		global $wpdb;
					...
					foreach ( $mailing_list as $data ) {
						if ( 'all' == $data['role'] ) { /* if were selected all users */
							...
							if ( array_key_exists( 'users_id', $data ) ) { /* if were selected specific users */
								if ( is_array( $data['users_id'] ) )
									$users_id .= empty( $users_id ) ? implode( ',', $data['users_id'] ) : ',' . implode( ',', $data['users_id'] );
								else
									$users_id .= empty( $users_id ) ? $data['users_id'] : ',' . $data['users_id'];
							}
						}
					}
				}
...
if ( ! function_exists( 'mlq_cron_mail' ) ) {
	function mlq_cron_mail() {
		...
		/*marking mailout as done if no mails in progress*/
		$active_mailouts = $wpdb->get_results( "SELECT `mail_send_id` FROM `" . $wpdb->base_prefix . "mlq_mail_send` WHERE `mail_status`=0;", ARRAY_A );
		foreach ( $active_mailouts as $current_mailout ) {
			$active_mailout_ids = $wpdb->get_col( "SELECT `id_mail` FROM `" . $wpdb->base_prefix . "mlq_mail_users` WHERE `status`=0;" );
			$error_mailout_ids = $wpdb->get_col( "SELECT `id_mail` FROM `" . $wpdb->base_prefix . "mlq_mail_users` WHERE `status`=2;" );
			if ( ! in_array( $current_mailout['mail_send_id'], $active_mailout_ids ) ) {
				if ( ! in_array( $current_mailout['mail_send_id'], $error_mailout_ids ) ) {
					$wpdb->update(
						$wpdb->base_prefix . 'mlq_mail_send',
						array(
							'mail_status'	=> 1
						...
						array(
							'mail_send_id'	=> $current_mailout['mail_send_id']
						)
					);
				} else {
					$wpdb->update(
		...
		if ( ! empty( $users_mail_sends ) ) {
			...
			foreach ( $users_mail_sends as $users_mail_send ) {
				...
				if ( $users_mail_send['plugin_id'] == $sender_plugin_id ) {
					...
					if ( array_key_exists( $sender_pro_plugin['plugin_link'], $plugins_list ) ) {
						...
						$content     = sndr_replace_shortcodes( $current_user_data, $letter_data );
						$fonts       = sndr_get_fonts( $letter_data['fonts'] );
						/* get letter data */
						$body           = '';
						...
						if ( '1' == $sndr_options['html_email'] ) { /* send html version of letter */
							$body = $content;
							$body .= "<img src=\"" . plugins_url( "files/get-view.php", 'sender-pro/sender-pro.php' ) . "?get_mes=" . $user_data['mail_users_id'] . "&s=" . md5( 'bws' . $user_data['mail_users_id'] . 'sndr_send' ) .  "\" width=\"0\" height=\"0\" />";
				...
				/* prepare mail data for php mailing */
				if ( 'wp_mail' != $mlq_options['mail_method'] ) {
					/* Adjust headers for phpmail mail method */
			...
			if ( ! empty( $sended ) ) {
				foreach( $sended as $send ) {
					...
					$wpdb->update(
						...
						array(
							'mail_users_id'	=> $send['mail_users_id']
						),
						array(
							'%d',
							'%d'
					...
					if ( $send['plugin_id'] == $wpdb->get_var( "SELECT `mail_plugin_id` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `plugin_link`='sender/sender.php';" ) ) {
						...
						if ( is_plugin_active( 'sender/sender.php' ) ) {
							...
							$wpdb->update(
								...
								array(
									'id_mail'	=> $send['mail_id_in_sender'],
									'id_user'	=> $send['user_id_in_sender']
								),
								array( '%d', '%d' ),
								array( '%d', '%d' )
							);
							...
							if ( empty( $sender_mails ) ) {
								...
								$wpdb->update(
									...
									array(
										'mail_send_id' => $send['mail_id_in_sender']
									)
								);
							}
						}
					...
					if ( "" != $send['attachment_path'] ) {
						...
						if ( empty( $cntctfrm_options ) )
							$cntctfrm_options = get_option( 'cntctfrm_options' );
						if ( isset( $cntctfrm_options['delete_attached_file'] ) && '1' == $cntctfrm_options['delete_attached_file'] ) {
			...
			if ( ! empty( $errors ) ) {
				$max_try_count = $mlq_options['max_try_count'];
				foreach ( $errors as $error ) {
					$mailing_try = $error['try'] + 1;
					if ( $mailing_try < $max_try_count ) {
						$wpdb->update(
							$wpdb->base_prefix . 'mlq_mail_users',
							array(
								'try'			=> $mailing_try
							...
							array(
								'mail_users_id'	=> $error['mail_users_id']
							)
						);
					} else {
						$wpdb->update(
					...
					if ( $error['plugin_id'] == $wpdb->get_var( "SELECT `mail_plugin_id` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `plugin_link`='sender/sender.php';" ) ) {
						...
						if ( is_plugin_active( 'sender/sender.php' ) ) {
							...
							$wpdb->update(
								...
								array(
									'id_mail'	=> $error['mail_id_in_sender'],
									'id_user'	=> $error['user_id_in_sender']
								)
							);
						}
					}
...
if ( ! function_exists( 'mlq_update_sender_pro_mail_data' ) ) {
	function mlq_update_sender_pro_mail_data( $mailout, $try_counter, $sent_or_not ) {
		...
		if ( $sent_or_not ) { /* if letter was sent successfully */
			...
			if ( $sndr_options['max_try_count'] == $try_counter ) {
				$wpdb->update(
					$wpdb->base_prefix . 'sndr_users',
					array(
						'try'    => $try_counter,
...
if ( ! function_exists( 'mlq_cron_mail_clear' ) ) {
	function mlq_cron_mail_clear() {
		...
		if ( $mlq_options['delete_old_mail'] ) {
			...
			$mails_to_delete = $wpdb->get_col( "SELECT `mail_send_id` FROM `" . $wpdb->base_prefix . "mlq_mail_send` WHERE ( `mail_status`=1 OR `mail_status`=2 ) AND `date_create`<" . $delete_from .";");
		...
		class MLQ_Plugin_List extends WP_List_Table {
			...
			*/
			function prepare_items() {
				...
				if ( ! in_array( $plugin_status, array( 'all', 'in_queue', 'not_in_queue', 'install', 'not_isntall', 'not_read_messages', 'active', 'inactive' ) ) )
					$plugin_status = 'all';
			...
			*/
			...
			function get_columns() {
				$columns = array(
					...
					'in_queue_status'	=> __( 'In queue', 'email-queue' ),
			...
			function get_sortable_columns() {
				$sortable_columns = array(
					...
					'in_queue_status'	=> array( 'in_queue_status', false ),
			...
			*/
			function extra_tablenav( $which ) {
				global $all_count, $in_queue_count, $not_in_queue_count, $install_count, $not_isntall_count, $active_count, $inactive_count; ?>
				<ul class="subsubsub">
					<li><a class="mlq-filter<?php if ( ! isset( $_REQUEST['plugin_status'] ) || ( isset( $_REQUEST['plugin_status'] ) && 'all' == $_REQUEST['plugin_status'] ) ) echo " current"; ?>" href="?page=mlq_settings"><?php _e( 'All', 'email-queue' ); ?><span class="mlq-count"> ( <?php echo $all_count; ?> )</span></a> | </li>
					<li><a class="mlq-filter<?php if( isset( $_REQUEST['plugin_status'] ) && "in_queue" == $_REQUEST['plugin_status'] ) echo " current"; ?>" href="?page=mlq_settings&plugin_status=in_queue"><?php _e( 'In queue', 'email-queue' ); ?><span class="mlq-count"> ( <?php echo $in_queue_count; ?> )</span></a> | </li>
			...
			/**
			 * Function to reorder tablenav, bulk actions and pagination os plugin's settings page
			 * @return void
			 */
			...
			function display_tablenav( $which ) {
				...
				<div class="tablenav <?php echo esc_attr( $which ); ?>">
					<?php if ( $this->has_items() ) { ?>
						<div class="alignleft clear actions bulkactions">
			...
			function column_title( $item ) {
				global $plugin_status;
				$actions = array();
				if ( '1' == $item['in_queue_status_value'] ) {
			...
			function plugin_list() {
				...
				if ( isset( $_REQUEST['plugin_status'] ) ) {
					switch ( $_REQUEST['plugin_status'] ) {
						...
						case 'not_in_queue':
							...
							break;
						case 'install':
							$sql_query .= "WHERE `install_status`=1";
							break;
						case 'not_isntall':
							$sql_query .= "WHERE `install_status`=0";
							break;
						case 'active':
							$sql_query .= "WHERE `install_status`=1 AND `active_status`=1";
							break;
						case 'inactive':
							$sql_query .= "WHERE `install_status`=1 AND `active_status`=0";
							break;
						default:
							$sql_query .= "";
							break;
				...
				foreach ( $plugins_data as $plugin ) {
					...
					if ( $plugin['in_queue_status'] && $plugin['active_status'] && $plugin['install_status'] )
						$plugins_list[$i]['title'] = '<b>' . $plugins_list[$i]['title'] . '</b>';
					/* get install and active status */
					$plugins_list[$i]['in_queue_status']	= ( '1' == $plugin['in_queue_status'] ) ? __( 'Yes', 'email-queue' ) : __( 'No', 'email-queue' );
					$plugins_list[$i]['in_queue_status_value'] = $plugin['in_queue_status'];
					...
					if ( '1' == $plugin['active_status'] && '1' == $plugin['in_queue_status'] )	{
						...
						/* if not in queue */
						global $plugin_status;
						$plugins_list[$i]['priority_general'] = __( 'You should', 'email-queue' ) . '&nbsp;<a href="' . wp_nonce_url( '?page=mlq_settings&action=restore_plugin_to_queue&plugin_id=' . $plugin['mail_plugin_id'] . '&plugin_status=' . $plugin_status, 'plugin_in_' . $plugin['mail_plugin_id'] ) . '">' . __( 'restore', 'email-queue' ) . '</a>&nbsp;' . __( 'plugin back to queue to set a priority', 'email-queue' );
					} else {
			...
			public function items_count() {
				global $wpdb, $all_count, $in_queue_count, $not_in_queue_count, $install_count, $not_isntall_count, $active_count, $inactive_count;
				$all_count = $in_queue_count = $not_in_queue_count = $install_count = $not_isntall_count = $active_count = $inactive_count = 0;
				...
				foreach ( $plugins_data as $plugin ) {
					...
					if ( '0' == $plugin['pro_status'] || '1' == $plugin['pro_status'] && empty( $plugin_pro ) || '2' == $plugin['pro_status'] && empty( $plugin_free ) || '1' == $plugin['pro_status'] && ! empty( $plugin_pro ) ) {
						...
						if ( '1' != $plugin['install_status'] )
							$not_isntall_count++;
						if ( '1' == $plugin['install_status'] && '1' == $plugin['active_status'] )
							$active_count++;
						if ( '1' == $plugin['install_status'] && '1' != $plugin['active_status'] )
							$inactive_count++;
						$all_count++;
...
if ( ! function_exists( 'mlq_check_plugins_versions' ) ) {
	function mlq_check_plugins_versions( $action_message ) {
		...
		/* get an array with info on default plugins */
		$default_plugins 		= mlq_get_default_plugins();
		/* get string var for our incompatible plugins */
		...
		foreach ( $default_plugins as $plugin_link => $plugin_data ) {
			...
			if ( isset( $plugin_data['plugin_function'] ) && in_array( $plugin_link, $mlq_active_plugin_list ) ) {
				...
				if ( ! function_exists( $plugin_data['plugin_function'] ) ) {
					...
					__( 'that is incompatible with Email Queue plugin. Please check for', 'email-queue' ) . '&nbsp;' .
				...
				switch ( $action ) {
					case 'remove_plugin_from_queue':
						if ( check_admin_referer( 'plugin_out_' . $_GET['plugin_id'] ) ) {
							$plugin = ( isset( $_GET['plugin_id'] ) && preg_match( '/^\d+$/', $_GET['plugin_id'] ) ) ? intval( $_GET['plugin_id'] ) : 0;
							if ( empty( $plugin ) ) {
								$action_message['error'] = $message_list['empty_plugins_list'];
							} else {
								$parallel_plugin_id = $wpdb->get_var( $wpdb->prepare(
										"SELECT `mail_plugin_id`
										FROM `" . $wpdb->base_prefix . "mlq_mail_plugins`
										WHERE `parallel_plugin_link`=(
											SELECT `plugin_link` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `mail_plugin_id` = %d
										);",
										$plugin
									)
								);
								if ( ! empty( $parallel_plugin_id ) ) {
									$wpdb->update(
										...
										array(
											'mail_plugin_id'	=> $parallel_plugin_id
										),
										array(
											'%d'
										),
					...
					case 'remove_plugins_from_queue':
						if ( check_admin_referer( plugin_basename( __FILE__ ), 'mlq_nonce_name' ) ) {
							$plugin_ids = ( isset( $_POST['plugin_id'] ) && is_array( $_POST['plugin_id'] ) ) ? $_POST['plugin_id'] : array();
							if ( ! $plugin_ids ) {
								$action_message['error'] = $message_list['empty_plugins_list'];
							} else {
								foreach ( $plugin_ids as $plugin ) {
									...
									$parallel_plugin_id = $wpdb->get_var( $wpdb->prepare( "SELECT `mail_plugin_id` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `parallel_plugin_link`=(
										SELECT `plugin_link` FROM `" . $wpdb->base_prefix . "mlq_mail_plugins` WHERE `mail_plugin_id`= %d);", $plugin ) );
									if ( ! empty( $parallel_plugin_id ) ) {
										$wpdb->update(
											$wpdb->base_prefix . "mlq_mail_plugins",
											array(
					...
					case 'restore_plugins_to_queue':
						if ( check_admin_referer( plugin_basename( __FILE__ ), 'mlq_nonce_name' ) ) {
							if ( empty( $_POST['plugin_id'] ) || ! is_array( $_POST['plugin_id'] ) ) {
								$action_message['error'] = $message_list['empty_plugins_list'];
							} else {
...
if ( ! function_exists( 'mlq_admin_settings_content' ) ) {
	function mlq_admin_settings_content() {
		global $wpdb, $mlq_options, $mlq_options_default, $title, $mlq_message, $mlq_error, $mlq_plugin_list, $mlq_active_plugin_list, $mlq_plugin_info;
		$message = '';
		...
		if ( isset( $_POST['mlq_form_submit'] ) && isset( $_POST['mlq_button_form_submit'] ) && check_admin_referer( $plugin_basename, 'mlq_nonce_name' ) ) {
			...
			if ( $mlq_options['display_options'] ) {
				...
				if ( isset( $_POST['mlq_mail_run_time'] ) ) {
					if ( empty( $_POST['mlq_mail_run_time'] ) || 1 > intval( $_POST['mlq_mail_run_time'] ) || ( ! preg_match( '/^\d+$/', $_POST['mlq_mail_run_time'] ) ) ) {
						...
						$mlq_options['mail_run_time'] = intval( $_POST['mlq_mail_run_time'] );
					}
					add_filter( 'cron_schedules', 'mlq_more_reccurences' );
				} else {
				...
				if ( isset( $_POST['mlq_mail_send_count'] ) ) {
					if ( empty( $_POST['mlq_mail_send_count'] ) || 1 > intval( $_POST['mlq_mail_send_count'] ) || ( ! preg_match( '/^\d+$/', $_POST['mlq_mail_send_count'] ) ) ) {
						...
						$mlq_options['mail_send_count'] = intval( $_POST['mlq_mail_send_count'] );
					}
				} else {
					$mlq_options['mail_send_count'] = $mlq_options_default['mail_send_count'];
				}
				/* check value from "Maximum number of tries" option */
				if ( isset( $_POST['mlq_max_try_count'] ) ) {
					if ( empty( $_POST['mlq_max_try_count'] ) || 1 > intval( $_POST['mlq_max_try_count'] ) || ( ! preg_match( '/^\d+$/', $_POST['mlq_max_try_count'] ) ) ) {
						$mlq_options['max_try_count'] = '5';
					} else {
						if ( 10 < $_POST['mlq_max_try_count'] ) {
							$message .= __( 'You may have entered too large a value in the "Maximum number of tries" option. Check please.', 'email-queue' ) . '<br />';
						}
						$mlq_options['max_try_count'] = intval( $_POST['mlq_max_try_count'] );
					}
				} else {
					$mlq_options['max_try_count'] = $mlq_options_default['max_try_count'];
				}
				/* set mail method */
				$mlq_options['mail_method'] = ( isset( $_POST['mlq_mail_method'] ) && 'mail' == $_POST['mlq_mail_method'] ) ? 'mail' : 'wp_mail';
				...
				if ( isset( $_POST['mlq_delete_old_mail'] ) ) {
					...
					if ( isset( $_POST['mlq_delete_old_mail_days'] ) ) {
						if ( empty( $_POST['mlq_delete_old_mail_days'] ) || 1 > intval( $_POST['mlq_delete_old_mail_days'] ) || ( ! preg_match( '/^\d+$/', $_POST['mlq_delete_old_mail_days'] ) ) ) {
							...
							$mlq_options['delete_old_mail_days'] = intval( $_POST['mlq_delete_old_mail_days'] );
						}
					} else {
		...
		/* update info on plugins before display and get list of active plugins */
		$mlq_plugin_list->check_plugin_install_and_active();

		if ( isset( $_REQUEST['bws_restore_confirm'] ) && check_admin_referer( $plugin_basename, 'bws_settings_nonce_name' ) ) {
			$mlq_options = $mlq_options_default;
			update_option( 'mlq_options', $mlq_options );
			$message = __( 'All plugin settings were restored.', 'email-queue' );
		}

		if ( ! isset( $_REQUEST['bws_restore_default'] ) && ! isset( $_REQUEST['bws_restore_confirm'] ) ) {
		...
		<div class="wrap mlq-report-list-page mlq-mail" id="mlq-mail">
			<h1><?php _e( 'Email Queue Settings', 'email-queue' ); ?></h1>
			...
			<?php if ( isset( $_REQUEST['bws_restore_default'] ) && check_admin_referer( $plugin_basename, 'bws_settings_nonce_name' ) ) {
				...
				<div class="error below-h2" <?php if ( empty( $mlq_error ) ) { echo 'style="display:none"'; } ?>><p><strong><?php echo $mlq_error; ?></strong></div>
				<div class="updated below-h2" <?php if ( empty( $mlq_message ) ) echo 'style="display: none;"'?>><p><?php echo $mlq_message ?></p></div>
				<style>
				...
				<form class="bws_form" method="post" action="admin.php?page=mlq_settings">
					<h2><?php _e( 'Plugins with email function', 'email-queue' ); ?></h2>
					...
					<table id="mlq-settings-table" class="form-table">
						...
						<tr class="mlq_ad_opt">
							...
							<td><input id="mlq_mail_run_time" name='mlq_mail_run_time' type='number' min='1' max='1000000' value='<?php echo $mlq_options['mail_run_time']; ?>' /> <?php _e( '(min)', 'email-queue' ); ?></td>
						</tr>
						<tr class="mlq_ad_opt">
							...
							<td>
								...
								<span class="bws_info">
									<?php $number = ( 60 > intval( $mlq_options['mail_run_time'] ) ) ? floor( ( 60 / intval( $mlq_options['mail_run_time'] ) ) ) * intval( $mlq_options['mail_send_count'] ) : intval( $mlq_options['mail_send_count'] );
						...
						<tr class="mlq_ad_opt">
							<th><?php _e( 'Maximum number of tries', 'email-queue' ); ?></th>
							<td>
								<input id="mlq_max_try_count" name='mlq_max_try_count' type='number' max='1000' value='<?php echo $mlq_options['max_try_count']; ?>'><br />
								<span class="bws_info">
									<?php _e( 'Maximum number of failed attempts to send emails', 'email-queue' );?>
								</span>
							</td>
						</tr>
						<tr class="mlq_ad_opt">
							...
							<td>
								<label>
									...
									<?php echo 'Wp-mail'; ?> <span class="bws_info">(<?php _e( 'You can use the wp_mail function for mailing', 'email-queue' ); ?>)</span>
								</label><br />
								<label>
									...
									<?php echo 'Mail'; ?> <span class="bws_info">(<?php _e( 'To send mail you can use the php mail function', 'email-queue' ); ?>)</span>
								</label>
							</td>
						</tr>
						<tr class="mlq_ad_opt">
							...
							<td>
								...
								<label  class="mlq_ad_opt mlq_delete_old_mail_option">
									<span class="bws_info"><?php _e( 'Delete messages that are older than this number of days', 'email-queue' ); ?></span><br />
					...
					<p class="submit">
						...
						<input  id="bws-submit-button" type="submit" class="button-primary" name="mlq_button_form_submit" value="<?php _e( 'Save changes', 'email-queue' ); ?>" />
					</p>
					<?php wp_nonce_field( $plugin_basename, 'mlq_nonce_name' ); ?>
				</form>
	...
	if ( ! class_exists( 'MLQ_Mail_Queue_List' ) ) {
		class MLQ_Mail_Queue_List extends WP_List_Table {
			...
			function prepare_items() {
				...
				$per_page			= $this->get_items_per_page( 'reports_per_page', 15 );
				$current_page		= $this->get_pagenum();
				$total_items		= $this->items_count();
				$this->set_pagination_args( array(
						'total_items'	=> $total_items,
			...
			*/
			function extra_tablenav( $which ) {
				...
				$filters_count = $wpdb->get_results (
					"SELECT COUNT(`mail_send_id`) AS `all`,
						( SELECT COUNT(`mail_send_id`) FROM " . $wpdb->base_prefix . "mlq_mail_send WHERE ( `mail_status`=1 OR `mail_status`=2 ) AND `trash_status`=0 ) AS `done`,
			...
			/**
			 * Function to reorder tablenav, bulk actions and pagination os plugin's page
			 * @return void
			 */
			...
			function column_title( $item ) {
				...
				$actions = array();
				if ( in_array( $mail_status, array( '&mail_status=all', '&mail_status=in_progress', '&mail_status=done' ) ) ) {
					$actions['show_report']  = '<a class="mlq-show-users-list" href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $item['id'] . '&list_paged=0&list_per_page=15' . $mail_status, 'show_mail_' . $item['id'] ) . '">' . __( 'Mail details', 'email-queue' ) . '</a>';
			...
			function single_row( $mail ) {
				...
				} elseif( preg_match( '/done-errors-status/', $mail['status'] ) ) {
					$row_class = 'mlq-done-errors-row';
			...
			function mail_list() {
				global $wpdb;
				$i 					= 0;
				$done_status 		= '<span class="mlq-done-status" title="' . __( 'All Done', 'email-queue' ) . '">' . __( 'Done', 'email-queue' ) . '</span>';
				$done_errors_status 		= '<span class="mlq-done-errors-status" title="' . __( 'Completed with errors', 'email-queue' ) . '">' . __( 'Completed with errors', 'email-queue' ) . '</span>';
				$in_progress_status = '<span class="mlq-inprogress-status" title="' . __( 'In progress', 'email-queue' ) . '">' . __( 'In progress', 'email-queue' ) . '</span>';
				$mails_list 		= array();
					...
					if ( isset( $_REQUEST['mail_status'] ) ) {
						switch ( $_REQUEST['mail_status'] ) {
							case 'in_progress':
								...
								break;
							case 'done':
								$sql_query .= "WHERE ( `mail_status`=1 OR `mail_status`=2 ) AND `trash_status`=0";
								break;
							case 'trash':
				...
				foreach ( $mails_data as $mail ) {
					...
					if ( '1' == $mail['mail_status'] ) {
						$mails_list[$i]['status'] = $done_status;
					} elseif ( '2' == $mail['mail_status'] ) {
						$mails_list[$i]['status'] = $done_errors_status;
					} else {
						$mails_list[$i]['status'] = $in_progress_status;
					}
					...
					$mails_list[$i]['date']		= date_i18n( 'd M Y H:i', $mail['date_create'], get_option('gmt_offset') * 3600 );
			...
			public function show_status( $mail_id ) {
				global $wpdb;
				$total_count = $send_count = $fail_count = $status = null;
				$count_mail = $wpdb->get_results(
					"SELECT COUNT(`id_mail`) AS `total`,
						...
						( SELECT COUNT(`id_mail`) FROM `" . $wpdb->base_prefix . "mlq_users` WHERE `id_mail`=" . $mail_id . " AND `status`=2 ) AS `fail`
				...
				if ( ! empty( $count_mail ) ) {
					...
					if ( $total_count == $send_count ) {
						$status = '<span class="mlq-done-status" title="' . __( 'All Done', 'email-queue' ) . '">' . __( 'Completed', 'email-queue' ) . '</span>';
					} elseif ( $total_count == $send_count + $fail_count ) {
						$status = '<span class="mlq-done-errors-status" title="' . __( 'Completed with errors', 'email-queue' ) . '">' . __( 'Completed with errors', 'email-queue' ) . '</span>';
					} else {
				...
				if ( isset( $_REQUEST['action'] ) && 'show_report' == $_REQUEST['action'] && $mail_id == $_REQUEST['report_id'] && check_admin_referer( 'show_mail_' . $_REQUEST['report_id'] ) ) {
					...
					if ( isset( $_POST['set_list_per_page_top'] ) || isset( $_POST['set_list_per_page_bottom'] ) ) { /* query from mail receivers pagination blocks */
						...
						/* cheeck if user want to change number of page in text field */
							...
							$per_page   = intval( $_REQUEST['list_per_page'] );
							$paged      = intval( $_POST['current_page'] );
						}
					...
					if ( ! empty( $users_list ) ) {
						$list_table =
							'<table class="mlq-receivers-list">
								<thead>
									...
									<tr class="mlq-receivers-list-headers">
										<td class="mlq-username"><a href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail . '&list_paged=' . $paged . '&list_per_page=' . $per_page . '&list_order_by=user_email&list_order=' . $list_order . $mail_status, 'show_mail_' . $mail ) . '">' . __( 'Receiver&#39;s email', 'email-queue' ) . '</a></td>
										<td><a href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail . '&list_paged=' . $paged . '&list_per_page=' . $per_page . '&list_order_by=status&list_order=' . $list_order . $mail_status, 'show_mail_' . $mail ) . '">' . __( 'Status', 'email-queue' ) . '</a></td>
										<td style="display: none;"><a href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail . '&list_paged=' . $paged . '&list_per_page=' . $per_page . '&list_order_by=view&list_order=' . $list_order . $mail_status, 'show_mail_' . $mail ) . '">' . __( 'View', 'email-queue' ) . '</a></td>
										<td>' . __( 'Try', 'email-queue' ) . '</td>
									</tr>
								</thead>
								<tbody>';
						foreach( $users_list as $list ) {
							$wp38_try_span = ( version_compare( get_bloginfo('version'), '3.9', '<' ) ) ? '<span class="mlq-try hidden">' . __( 'Try', 'email-queue' ) . '</span> ' : '';
							$user_email = $list->user_email;
							if ( empty( $user_email ) ) {
								$user_email = '<i>- ' . __( 'Receiver&#39;s email not found', 'email-queue' ) . ' -</i>';
							}
							$list_table .= '<tr>
												<td class="mlq-username" data-colname="' . __( 'Receiver&#39;s email', 'email-queue' ) . '"><span>' . $user_email . '</span></td>
												<td data-colname="' . __( 'Status', 'email-queue' ) . '">';
							if( '1' == $list->status ) {
								$list_table .= '<span style="color: #006505;">' . __( 'sent', 'email-queue' ) . '</span>';
							} elseif( '2' == $list->status ) {
								$list_table .= '<span style="color: #700;">' . __( 'failed', 'email-queue' ) . '</span>';
							} else {
								$list_table .= '<span style="color: #700;">' . __( 'in queue', 'email-queue' ) . '</span>';
							}
							$list_table .=		'</td>
												<td class="hidden" style="display: none;">';
							if( '1' == $list->view ) {
								$list_table .= '<span style="color: green;">' . __( 'read', 'email-queue' ) . '</span>';
							} else {
								$list_table .= '<span style="color: #555;">' . __( 'not read', 'email-queue' ) . '</span>';
							}
							$list_table .=	'</td>
											<td data-colname="' . __( 'Try', 'email-queue' ) . '">' . $wp38_try_span . '<span>' . $list->try . '</span></td>
										</tr>';
						}
						$list_table .=
								'</tbody>
						...
						/* if( empty( $users_list ) ) */
						$list_table = '<span style="color:red;">' . __( "The list of mail receivers can't be found.", 'email-queue' ) . '</span>';
					}
				}
				return $list_table;
			}
			...
			function mail_receivers_pagination( $mail_id, $per_page, $paged, $list_order_by, $list_order, $show_hidden, $place ) {
				...
				if ( intval( $users_count ) > $per_page ) {
					...
					if ( 0 < $paged ) { /* if this is NOT first page of mail receivers list */
						...
						$pagination_block .=
							'<a class="first-page" href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail_id . '&list_paged=0&list_per_page=' . $per_page . $mail_status . '&list_order_by=' . $list_order_by . '&list_order=' . $list_order, 'show_mail_' . $mail_id ) . '" title="' . __( 'Go to the first page', 'email-queue' ) . '">&laquo;</a>
							<a class="previous-page" href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail_id . '&list_paged=' . $previous_page_link . '&list_per_page=' . $per_page . $mail_status . '&list_order_by=' . $list_order_by . '&list_order=' . $list_order, 'show_mail_' . $mail_id ) . '" title="' . __( 'Go to the previous page', 'email-queue' ) . '">&lsaquo;</a>';
					...
					if ( $show_hidden ) {
						$pagination_block .=
							...
							wp_nonce_field( 'show_mail_' . $mail_id );
					}
					...
					if ( $paged < $total_pages ) { /* if this is NOT last page */
						...
						$pagination_block .=
							'<a class="next-page" href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail_id . '&list_paged=' . $next_page_link . '&list_per_page=' . $per_page . $mail_status . '&list_order_by=' . $list_order_by . '&list_order=' . $list_order, 'show_mail_' . $mail_id ) . '" title="' . __( 'Go to the next page', 'email-queue' ) . '">&rsaquo;</a>
							<a class="last-page" href="' . wp_nonce_url( '?page=mlq_view_mail_queue&action=show_report&report_id=' . $mail_id . '&list_paged=' . $total_pages . '&list_per_page=' . $per_page . $mail_status . '&list_order_by=' . $list_order_by . '&list_order=' . $list_order, 'show_mail_' . $mail_id ) . '" title="' . __( 'Go to the last page', 'email-queue' ) . '">&raquo;</a>';
...
if ( ! function_exists( 'mlq_mail_view' ) ) {
	function mlq_mail_view() {
		...
		$wp38_class = version_compare( get_bloginfo('version'), '3.9', '<' ) ? ' mlq-wp38' : ''; ?>
		<div class="wrap mlq-report-list-page<?php echo $wp38_class; ?>">
			<h1><?php _e( 'Email Queue', 'email-queue' ); ?></h1>
...
if ( ! function_exists ( 'mlq_admin_notices' ) ) {
	function mlq_admin_notices() {
		global $hook_suffix, $mlq_plugin_info, $mlq_options;

		if ( 'plugins.php' == $hook_suffix ) {
			if ( is_multisite() && ! is_network_admin() && is_admin() ) { ?>
				<div class="update-nag"><strong><?php _e( 'Notice:', 'email-queue' ); ?></strong>
					<?php if ( is_plugin_active_for_network( plugin_basename( __FILE__ ) ) ) {
						_e( 'Due to the peculiarities of the multisite work, Email Queue plugin has only', 'email-queue' ); ?> <a target="_blank" href="<?php echo network_admin_url( 'admin.php?page=mlq_settings' ); ?>"><?php _e( 'Network settings page', 'email-queue' ); ?></a>
					<?php } else {
						_e( 'Due to the peculiarities of the multisite work, Email Queue plugin has the network settings page only and it should be Network Activated. Please', 'email-queue' ); ?> <a target="_blank" href="<?php echo network_admin_url( 'plugins.php' ); ?>"><?php _e( 'Activate Email Queue for Network', 'email-queue' ); ?></a>
					<?php } ?>
				</div>
			<?php }
		}
		...
		$plugin_pages = array( 'mlq_view_mail_queue', 'mlq_settings' );
		if ( in_array( $hook_suffix, $admin_pages ) || ( isset( $_GET['page'] ) && in_array( $_GET['page'], $plugin_pages ) ) ) {
			if ( empty( $mlq_options ) )
				mlq_register_settings();

			if ( ! $mlq_options['display_not_supported_notice'] )
				return;

			if ( isset( $_POST['bws_hide_not_supported_notice'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'bws_settings_nonce_name' ) ) {
				$mlq_options['display_not_supported_notice'] = 0;
				update_option( 'mlq_options', $mlq_options );
				return;
			} ?>
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				<div class="bws_banner_on_plugin_page bws_banner_to_settings">
					<div class="icon">
						<img title="" src="<?php echo plugins_url( 'images/email-queue-icon.png', __FILE__ ); ?>" alt="" />
					</div>
					<div class="text">
						<strong><?php printf( __( '%s will be no longer supported', 'bestwebsoft' ), 'Email Queue' ); ?></strong>
						<br />
						<?php printf( __( '%s plugin will be no longer supported (updates will be unavailable) starting from July 2017.', 'bestwebsoft' ), 'Email Queue' ); ?>
					</div>
...
if ( ! function_exists( 'mlq_send_uninstall' ) ) {
	function mlq_send_uninstall() {
		...
		$mlq_sql = "DROP TABLE `" . $base_prefix . "mlq_mail_send`, `" . $base_prefix . "mlq_mail_users`, `" . $base_prefix . "mlq_mail_plugins`;";
		$wpdb->query( $mlq_sql );
...
add_action( 'plugins_loaded', 'mlq_plugins_loaded' );

add_action( 'admin_enqueue_scripts', 'mlq_admin_head' );
/* grab mail data from other plugins */
add_action( 'cntctfrm_get_mail_data_for_mlq', 'mlq_get_mail_data_from_contact_form', 10, 7 );
add_action( 'sndr_get_data_start_mailout', 'mlq_start_mailout_from_sender_pro', 10, 2 );
...
add_action( 'admin_notices', 'mlq_admin_notices' );
add_action( 'network_admin_notices', 'mlq_admin_notices' );
